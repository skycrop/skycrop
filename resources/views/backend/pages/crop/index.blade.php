...
<div class="p-4 mx-auto max-w-(--breakpoint-2xl) md:p-6" x-data="{ selectedCrop: [], selectAll: false, bulkDeleteModalOpen: false }">
        ...
        <div class="rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="px-5 py-4 sm:px-6 sm:py-5 flex flex-col md:flex-row justify-between items-center gap-3">
                @include('backend.partials.search-form', [
                    'placeholder' => __('Search by crop name'),
                ])
               <div class="flex items-center gap-3">
                 <div class="flex items-center gap-2">
                    <!-- Bulk Actions dropdown -->
                    <div class="flex items-center justify-center" x-show="selectedCrop.length > 0">
                        <button id="bulkActionsButton" data-dropdown-toggle="bulkActionsDropdown" class="btn-secondary flex items-center justify-center gap-2 text-sm" type="button">
                            ...
                            <span>{{ __('Bulk Actions') }} (<span x-text="selectedCrop.length"></span>)</span>
                ...
                @if (auth()->user()->can('crop.create'))
                <a href="{{ route('admin.crop.create') }}" class="btn-primary flex items-center gap-2">
                ...
                <table id="dataTable" class="w-full dark:text-gray-300">
                            ...
                            <th width="5%" class="p-2 bg-gray-50 dark:bg-gray-800 dark:text-white text-left px-5 sm:px-6">
                                <div class="flex items-center">
                                    <input
                                        ...
                                        @click="
                                            selectAll = !selectAll;
                                            selectedCrop = selectAll ?
                                                [...document.querySelectorAll('.tag-checkbox')].map(cb => cb.value) :
                                                [];
                                        "
                                    >
                                </div>
                    ...
                    <tbody>
                        @forelse ($crops as $crop)
                            <tr class="{{ $loop->index + 1 != count($crops) ?  'border-b border-gray-100 dark:border-gray-800' : '' }}">
                                <td class="px-5 py-4 sm:px-6">
                                    <input
                                        type="checkbox"
                                        class="tag-checkbox form-checkbox h-4 w-4 text-primary border-gray-300 rounded focus:ring-primary dark:focus:ring-primary dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                                        value="{{ $crop->id }}"
                                ...
                                <td class="px-5 py-4 sm:px-6 flex justify-center">
                                    <x-buttons.action-buttons :label="__('Actions')" :show-label="false" align="right">
                                        @if (auth()->user()->can('crop.edit'))
                                            <x-buttons.action-item
                                                :href="route('admin.crop.edit', $crop->id)"
                                                icon="pencil"
                                                :label="__('Edit')"
                                            />
                                        @endif
                                        {!! ld_apply_filters('crop_actions_after_edit', '', $crop) !!}                                        

                                        @if (auth()->user()->can('crop.delete'))
                                            <div x-data="{ deleteModalOpen: false }">
                                                ...
                                                <x-modals.confirm-delete
                                                    id="delete-modal-{{ $crop->id }}"
                                                    title="{{ __('Delete') }} {{ strtolower($crop->name) }}"
                                                    content="{{ __('Are you sure you want to delete this') }} {{ strtolower($crop->name) }}?"
                                                    formId="delete-form-{{ $crop->id }}"
                                                    formAction="{{ route('admin.crop.destroy', $crop->id) }}"
                                        ...
                                        @endif
                                        {!! ld_apply_filters('crop_actions_after_delete', '', $crop) !!}
                        ...
                        @empty
                            <tr class="border-b border-gray-100 dark:border-gray-800">
                                <td colspan="5" class="px-5 py-4 sm:px-6 text-center">
                                    <span class="text-gray-500 dark:text-gray-300">{{ __('No crop found') }}</span>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        >
            ...
            <div class="flex items-center justify-end gap-3 border-t border-gray-100 p-4 dark:border-gray-800">
                <form id="bulk-delete-form" action="{{ route('admin.roles.bulk-delete') }}" method="POST">
                    ...
                    <template x-for="id in selectedCrop" :key="id">
