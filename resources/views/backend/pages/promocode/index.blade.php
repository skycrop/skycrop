        ...
        <div class="rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="px-5 py-4 sm:px-6 sm:py-5 flex flex-col md:flex-row justify-between items-center gap-3">
               ...
               <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.promocode.create') }}" class="btn-primary flex items-center gap-2">
                ...
                <table id="dataTable" class="w-full dark:text-gray-300">
                    <thead class="bg-light text-capitalize">
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            ...
                            <th width="12%" class="p-2 bg-gray-50 dark:bg-gray-800 dark:text-white">{{ __('Discount Type') }}</th>
                            <th width="12%" class="p-2 bg-gray-50 dark:bg-gray-800 dark:text-white">{{ __('Amount/Precent') }}</th>
                            <th width="12%" class="p-2 bg-gray-50 dark:bg-gray-800 dark:text-white">{{ __('Start Date') }}</th>
                            <th width="12%" class="p-2 bg-gray-50 dark:bg-gray-800 dark:text-white">{{ __('End Date') }}</th>
                            <th width="12%" class="p-2 bg-gray-50 dark:bg-gray-800 dark:text-white">{{ __('Status') }}</th>
                            <th width="12%" class="p-2 bg-gray-50 dark:bg-gray-800 dark:text-white">{{ __('Action') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($promocodes as $promo)
                            <tr class="{{ $loop->index + 1 != count($promocodes) ?  'border-b border-gray-100 dark:border-gray-800' : '' }}">
                                ...
                                <td class="px-5 py-4 sm:px-6">
                                        {{ $promo->start_date->format('F d, Y') }}
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                        {{ $promo->end_date->format('F d, Y') }}
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                        {{ $promo->is_active ? 'Active' : 'In Active' }}
                                </td>
                                <td class="px-5 py-4 sm:px-6 flex justify-center">
                                    <x-buttons.action-buttons :label="__('Actions')" :show-label="false" align="right">
                                        @if (auth()->user()->can('promocode.edit'))
                                            <x-buttons.action-item
                                                :href="route('admin.promocode.edit', $promo->id)"
                                                icon="pencil"
                                                :label="__('Edit')"
                                            />
                                        @endif
                                        {!! ld_apply_filters('brand_actions_after_edit', '', $promo) !!}                                        
                                        ...
                                        @if (auth()->user()->can('promocode.delete'))
                                            <!-- <div x-data="{ deleteModalOpen: false }">
                                                ...
                                                <x-modals.confirm-delete
                                                    id="delete-modal-{{ $promo->id }}"
                                                    title="{{ __('Delete') }} {{ strtolower($promo->name) }}"
                                                    content="{{ __('Are you sure you want to delete this') }} {{ strtolower($promo->name) }}?"
                                                    formId="delete-form-{{ $promo->id }}"
                                                    formAction="{{ route('admin.promocode.destroy', $promo->id) }}"
                                        ...
                                        @endif
                                        {!! ld_apply_filters('brand_actions_after_delete', '', $promo) !!}
                        ...
                        @empty
                            <tr class="border-b border-gray-100 dark:border-gray-800">
                                <td colspan="5" class="px-5 py-4 sm:px-6 text-center">
                                    <span class="text-gray-500 dark:text-gray-300">{{ __('No Code found') }}</span>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
