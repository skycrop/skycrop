        ...
        <div class="rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="px-5 py-4 sm:px-6 sm:py-5 flex flex-col md:flex-row justify-between items-center gap-3">
               ...
               <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.farmer.create') }}" class="btn-primary flex items-center gap-2">
                ...
                <table id="dataTable" class="w-full dark:text-gray-300">
                    ...
                    <tbody>
                        @forelse ($farmers as $farmer)
                            <tr class="{{ $loop->index + 1 != count($farmers) ?  'border-b border-gray-100 dark:border-gray-800' : '' }}">
                                ...
                                <td class="px-5 py-4 sm:px-6 flex justify-center">
                                    <x-buttons.action-buttons :label="__('Actions')" :show-label="false" align="right">
                                        @if (auth()->user()->can('farmer.edit'))
                                            <!-- <x-buttons.action-item
                                                :href="route('admin.farmer.edit', $farmer->id)"
                                                icon="pencil"
                                                :label="__('Edit')"
                                            /> -->

                                            <x-buttons.action-item
                                                :href="route('admin.farmer.listFarm', $farmer->id)"
                                                icon="pencil"
                                                :label="__('Farms List')"
                                            />

                                            <x-buttons.action-item
                                                :href="route('admin.farmer.createFarm', $farmer->id)"
                                                icon="pencil"
                                                :label="__('Add Farm')"
                                            />
                                        @endif
                                        {!! ld_apply_filters('farmer_actions_after_edit', '', $farmer) !!}                                        

                                        @if (auth()->user()->can('farmer.delete'))
                                            <div x-data="{ deleteModalOpen: false }">
                                                ...
                                                <x-modals.confirm-delete
                                                    id="delete-modal-{{ $farmer->id }}"
                                                    title="{{ __('Delete') }} {{ strtolower($farmer->name) }}"
                                                    content="{{ __('Are you sure you want to delete this') }} {{ strtolower($farmer->name) }}?"
                                                    formId="delete-form-{{ $farmer->id }}"
                                                    formAction="{{ route('admin.farmer.destroy', $farmer->id) }}"
                                        ...
                                        @endif
                                        {!! ld_apply_filters('farmer_actions_after_delete', '', $farmer) !!}
