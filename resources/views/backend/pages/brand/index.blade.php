        ...
        <div class="rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="px-5 py-4 sm:px-6 sm:py-5 flex flex-col md:flex-row justify-between items-center gap-3">
               ...
               <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.brand.create') }}" class="btn-primary flex items-center gap-2">
                ...
                <table id="dataTable" class="w-full dark:text-gray-300">
                    ...
                    <tbody>
                        @forelse ($brands as $brand)
                            <tr class="{{ $loop->index + 1 != count($brands) ?  'border-b border-gray-100 dark:border-gray-800' : '' }}">
                                ...
                                <td class="px-5 py-4 sm:px-6 flex justify-center">
                                    <x-buttons.action-buttons :label="__('Actions')" :show-label="false" align="right">
                                        @if (auth()->user()->can('brand.edit'))
                                            ...
                                            />
                                        @endif
                                        {!! ld_apply_filters('brand_actions_after_edit', '', $brand) !!}                                        
                                        ...
                                        @if (auth()->user()->can('brand.delete'))
                                            <div x-data="{ deleteModalOpen: false }">
                                                ...
                                                <x-modals.confirm-delete
                                                    id="delete-modal-{{ $brand->id }}"
                                                    title="{{ __('Delete') }} {{ strtolower($brand->name) }}"
                                                    content="{{ __('Are you sure you want to delete this') }} {{ strtolower($brand->name) }}?"
                                                    formId="delete-form-{{ $brand->id }}"
                                                    formAction="{{ route('admin.brand.destroy', $brand->id) }}"
                                        ...
                                        @endif
                                        {!! ld_apply_filters('brand_actions_after_delete', '', $brand) !!}
                        ...
                        @empty
                            <tr class="border-b border-gray-100 dark:border-gray-800">
                                <td colspan="5" class="px-5 py-4 sm:px-6 text-center">
                                    <span class="text-gray-500 dark:text-gray-300">{{ __('No brands found') }}</span>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
