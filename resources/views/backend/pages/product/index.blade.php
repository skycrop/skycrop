        ...
        <div class="rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="px-5 py-4 sm:px-6 sm:py-5 flex flex-col md:flex-row justify-between items-center gap-3">
                @include('backend.partials.search-form', [
                    'placeholder' => __('Search by product name, sku'),
                ])
               <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.product.create') }}" class="btn-primary flex items-center gap-2">
                ...
                <table id="dataTable" class="w-full dark:text-gray-300">
                    <thead class="bg-light text-capitalize">
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            <th width="5%" class="p-2 bg-gray-50 dark:bg-gray-800 dark:text-white text-left px-5 sm:px-6">
                                <div class="flex items-center">
                                    ...
                                    <input
                                        ...
                                        @click="
                                            selectAll = !selectAll;
                                            selectedBrand = selectAll ?
                                                [...document.querySelectorAll('.brand-checkbox')].map(cb => cb.value) :
                                                [];
                                        "
                                    >
                                    @else
                            ...
                            <th width="15%" class="p-2 bg-gray-50 dark:bg-gray-800 dark:text-white text-left px-5">{{ __('Brand') }}</th>
                            <th width="15%" class="p-2 bg-gray-50 dark:bg-gray-800 dark:text-white text-left px-5">{{ __('Category') }}</th>                            
                            <th width="12%" class="p-2 bg-gray-50 dark:bg-gray-800 dark:text-white ">{{ __('Action') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($products as $product)
                            <tr class="{{ $loop->index + 1 != count($products) ?  'border-b border-gray-100 dark:border-gray-800' : '' }}">
                                <td class="px-5 py-4 sm:px-6">
                                    @if (auth()->user()->can('product.delete'))
                                    <input
                                        type="checkbox"
                                        class="brand-checkbox form-checkbox h-4 w-4 text-primary border-gray-300 rounded focus:ring-primary dark:focus:ring-primary dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                                        value="{{ $product->id }}"
                                ...
                                <td class="px-5 py-4 sm:px-6 flex justify-center">
                                    <x-buttons.action-buttons :label="__('Actions')" :show-label="false" align="right">
                                        ...
                                        @if (auth()->user()->roles->contains('name', 'Supplier'))
                                            <div x-data="{ inventoryModalOpen: false }">
                                                <x-buttons.action-item
                                                    type="modal-trigger"
                                                    modal-target="inventoryModalOpen"
                                                    icon="pencil"
                                                    :label="__('Update Inventory')"
                                                    class="text-green-600 dark:text-green-400"
                                                />

                                                <x-modals.update-inventory
                                                    id="delete-modal-{{ $product->id }}"
                                                    title="{{ __('Update Inventory') }} : {{ strtolower($product->name) }}"
                                                    :variants="$product->variants"
                                                    :product="$product"
                                                    :currentInventory="$currentInventory"
                                                    formId="inventory-form-{{ $product->id }}"
                                                    formAction="{{ route('admin.product.update-inventory', $product->id) }}"
                                        ...
                                        {!! ld_apply_filters('product_actions_after_edit', '', $product) !!}                                        
                                        ...
                                        @if (auth()->user()->can('product.delete'))
                                            <div x-data="{ deleteModalOpen: false }">
                                                ...
                                                <x-modals.confirm-delete
                                                    id="delete-modal-{{ $product->id }}"
                                                    title="{{ __('Delete') }} {{ strtolower($product->name) }}"
                                                    content="{{ __('Are you sure you want to delete this') }} {{ strtolower($product->name) }}?"
                                                    formId="delete-form-{{ $product->id }}"
                                                    formAction="{{ route('admin.product.destroy', $product->id) }}"
                                        ...
                                        @endif
                                        {!! ld_apply_filters('product_actions_after_delete', '', $product) !!}
                        ...
                        @empty
                            <tr class="border-b border-gray-100 dark:border-gray-800">
                                <td colspan="5" class="px-5 py-4 sm:px-6 text-center">
                                    <span class="text-gray-500 dark:text-gray-300">{{ __('No products found') }}</span>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        >
            ...
            <div class="px-4 text-center">
                <p class="text-gray-500 dark:text-gray-300">
                    {{ __('Are you sure you want to delete the selected products?') }}
