...
@section('admin-content')
        ...
        <div class="space-y-6">
            <div class="rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="px-5 py-4 sm:px-6 sm:py-5">
                    ...
                    <form action="{{ route('admin.product.store') }}" method="POST" enctype="multipart/form-data" id="validateProductForm">
                        @csrf
                            ...
                            <div class="rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                                ...
                                <div class="p-5 space-y-6 border-t border-gray-100 dark:border-gray-800 sm:p-6">                    
                                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                        <div x-data="slugGenerator('{{ old('name') }}', '{{ old('slug') }}')">
                                            <div class="space-y-1">
                                                ...
                                                <input type="text" name="name" id="name" x-model="title" required autofocus
                                                    value="{{ old('name') }}" placeholder="{{ __('Enter Product Name') }}"
                                                    class="form-control">
                                            </div>                                            
                                        </div>
                                        ...
                                        <div>
                                            <x-inputs.combobox name="collection_ids[]" label="{{ __('Collection') }}"
                                                ...
                                                placeholder="{{ __('Select Collection') }}" :options="collect($collections)
                                                    ...
                                                    ->values()
                                                    ->toArray()" :selected="old('collection_ids', [])"
                                    ...
                                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-1 mb-6">
                                        <div class="space-y-1 mb-12">
                                            ...
                                            <textarea name="description" id="content" rows="4">{!! old('description') !!}</textarea>                                
                                        </div>
                                    </div>                    
                                </div>                
                                    ...
                                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                        <div>
                                            ...
                                            <select name="seed_type" class="form-control mt-1">
                                                ...
                                                <option value="{{ $seed }}">{{ ucfirst($seed) }}</option>
                                                @endforeach
                                            </select>  
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <select id="crop-select"  name="crop_id" class="form-control mt-1">
                                                ...
                                                <option value="{{ $crop->id }}">{{ ucfirst($crop->crop_name) }}</option>
                                                @endforeach
                                            </select>                                            
                                        </div>

                                        <div>
                                            <input type="hidden" name="suitable_season"      id="suitable_season">
                                            <input type="hidden" name="suitable_soil_types"  id="suitable_soil_types">
                                            <input type="hidden" name="suitable_land_types"  id="suitable_land_types">
                                            <input type="hidden" name="suitable_water_types" id="suitable_water_types">
                                            <input type="hidden" name="soil_ph_min"          id="soil_ph_min">
                                            <input type="hidden" name="soil_ph_max"          id="soil_ph_max">
                                            <input type="hidden" name="water_ph_min"         id="water_ph_min">
                                            <input type="hidden" name="water_ph_max"         id="water_ph_max">                                       
                                        </div>
                                    </div>
                                </div>                
                                    ...
                                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                                        <div class="space-y-1">
                                            ...
                                            <input type="text" name="video_urls[benefit]" id="video_urls_benefit" required autofocus
                                                value="{{ old('video_urls.benefit') }}" placeholder="{{ __('Enter Benefit Video URL') }}"                                                                        
                                                class="form-control">
                                        </div>
                                        ...
                                        <div class="space-y-1">
                                            ...
                                            <input type="text" name="video_urls[testimonial]" id="video_urls_testimonial" required autofocus
                                                value="{{ old('video_urls.testimonial') }}" placeholder="{{ __('Enter Testimonial Video URL') }}"                                                                        
                                                class="form-control">
                                        </div>
                                        ...
                                        <div class="space-y-1">
                                            ...
                                            <input type="text" name="video_urls[result]" id="video_urls_result" required autofocus
                                                value="{{ old('video_urls.result') }}" placeholder="{{ __('Enter Result Video URL') }}"                                                                        
                                                class="form-control">
                                        </div>
                                    </div>
                                </div>                
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        cropSelect.addEventListener('change', function () {
            ...
            if (cropId) {
                fetch(`/admin/crop/${cropId}/details`)
                    .then(res => res.json())
                    .then(data => {
                        document.getElementById('suitable_season').value      = data.suitable_season      ?? '';
                        document.getElementById('suitable_soil_types').value  = data.suitable_soil_types  ?? '';
                        document.getElementById('suitable_land_types').value  = data.suitable_land_types  ?? '';
                        document.getElementById('suitable_water_types').value = data.suitable_water_types ?? '';
                        document.getElementById('soil_ph_min').value          = data.soil_ph_min          ?? '';
                        document.getElementById('soil_ph_max').value          = data.soil_ph_max          ?? '';
                        document.getElementById('water_ph_min').value         = data.water_ph_min         ?? '';
                        document.getElementById('water_ph_max').value         = data.water_ph_max         ?? '';
                    })
                    .catch(err => {
        ...
        const container = document.querySelector('.variants-container'); // Adjust if needed

        container.addEventListener('change', function (e) {
            if (e.target.classList.contains('is-default-checkbox')) {
                // If this checkbox was checked
                if (e.target.checked) {
                    // Uncheck all others
                    const allDefaultCheckboxes = container.querySelectorAll('.is-default-checkbox');
