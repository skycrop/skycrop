...
<div class="p-4 mx-auto max-w-(--breakpoint-2xl) md:p-6" x-data="{ selectedTag: [], selectAll: false, bulkDeleteModalOpen: false }">
        ...
        <div class="rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="px-5 py-4 sm:px-6 sm:py-5 flex flex-col md:flex-row justify-between items-center gap-3">
                @include('backend.partials.search-form', [
                    'placeholder' => __('Search by chemical name'),
                ])
               <div class="flex items-center gap-3">
                 <div class="flex items-center gap-2">
                    <!-- Bulk Actions dropdown -->
                    <div class="flex items-center justify-center" x-show="selectedTag.length > 0">
                        <button id="bulkActionsButton" data-dropdown-toggle="bulkActionsDropdown" class="btn-secondary flex items-center justify-center gap-2 text-sm" type="button">
                            ...
                            <span>{{ __('Bulk Actions') }} (<span x-text="selectedTag.length"></span>)</span>
                ...
                @if (auth()->user()->can('chemical.create'))
                <a href="{{ route('admin.tag.create') }}" class="btn-primary flex items-center gap-2">
            ...
            <div class="space-y-3 border-t border-gray-100 dark:border-gray-800 overflow-x-auto overflow-y-visible">
                <table id="dataTable" class="w-full dark:text-gray-300">
                    ...
                    <tbody>
                        @forelse ($tags as $tag)
                            <tr class="{{ $loop->index + 1 != count($tags) ?  'border-b border-gray-100 dark:border-gray-800' : '' }}">
                                ...
                                <td class="px-5 py-4 sm:px-6 flex justify-center">
                                    <x-buttons.action-buttons :label="__('Actions')" :show-label="false" align="right">
                                        @if (auth()->user()->can('chemical.edit'))
                                            ...
                                            />
                                        @endif
                                        {!! ld_apply_filters('tag_actions_after_edit', '', $tag) !!}                                        

                                        @if (auth()->user()->can('chemical.delete'))
                                            <div x-data="{ deleteModalOpen: false }">
                                                ...
                                                <x-modals.confirm-delete
                                                    ...
                                                    title="{{ __('Delete') }} {{ strtolower($tag->name) }}"
                                                    content="{{ __('Are you sure you want to delete this') }} {{ strtolower($tag->name) }}?"
                                                    formId="delete-form-{{ $tag->id }}"
                                                    formAction="{{ route('admin.tag.destroy', $tag->id) }}"
                                        ...
                                        @endif
                                        {!! ld_apply_filters('tag_actions_after_delete', '', $tag) !!}
                        ...
                        @empty
                            <tr class="border-b border-gray-100 dark:border-gray-800">
                                <td colspan="5" class="px-5 py-4 sm:px-6 text-center">
                                    <span class="text-gray-500 dark:text-gray-300">{{ __('No chemical found') }}</span>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        >
            ...
            <div class="flex items-center justify-end gap-3 border-t border-gray-100 p-4 dark:border-gray-800">
                <form id="bulk-delete-form" action="{{ route('admin.roles.bulk-delete') }}" method="POST">
                    ...
                    <template x-for="id in selectedTag" :key="id">
