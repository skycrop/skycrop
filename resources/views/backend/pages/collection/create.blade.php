        ...
        <div class="space-y-6">
            <div class="rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="p-5 space-y-6 border-t border-gray-100 dark:border-gray-800 sm:p-6">
                    <form action="{{ route('admin.collection.store') }}" method="POST" enctype="multipart/form-data" id="validateCollectionForm">
                        @csrf
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">

                            <div class="space-y-1">
                                <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Collection Name') }} <span class="text-red-500">*</span></label>
                                <input type="text" name="name" id="name" required autofocus
                                    value="{{ old('name') }}" placeholder="{{ __('Enter Collection Name') }}"
                                    class="form-control">
                            </div>
                            ...
                            <div>
                                <x-inputs.combobox name="type_id" label="{{ __('Select Type (if any)') }}" 
                                    placeholder="{{ __('Select Collection Type') }}" 
                                    :options="collect($collectionsType)->map(function ($cat) {
                                        return [
                                            'value' => $cat->id,
                                            'label' => str_repeat('-- ', 0) . ucfirst($cat->name),
                                        ];
                                    })->values()->toArray()"
