...
const GOOGLE_MAPS_KEY = window.APP_CONFIG.googleMapsKey;
...
Alpine.data('variantFields', (initialMeta = {}) => {
    return {
        fields: [],
        initialized: false,
                ...
                this.fields = Object.entries(initialMeta).map(([key, data]) => {
                    if (typeof data === 'object' && data !== null) {
                        return {
                            ...
                            variant_pack_description: data.pack_description || '',
                            variant_sku: data.sku || '',
                            variant_item_price: data.mrp || '',
                            variant_selling_price: data.selling_price || '',
                            variant_stock_quantity: data.stock_quantity || '',
                            variant_is_default: data.is_default === 1 || data.is_default === true,
                            variant_is_available: data.is_available === 1 || data.is_available === true
                        };
                    } else {
                        // Handle legacy format where data is just the value
                        return {
...
document.addEventListener('DOMContentLoaded', function () {    
    Object.values(validations).forEach(init => init());
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function success(position) {        
        ...
        fetch(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lng}&key=${GOOGLE_MAPS_KEY}`)
            .then(response => response.json())
            .then(data => {
                if (data.status === "OK") {
                    let address = data.results[0].address_components;
                    let area = address.find(c => c.types.includes("sublocality"))?.long_name 
                            || address.find(c => c.types.includes("locality"))?.long_name 
                            || data.results[0].formatted_address;

                    document.getElementById('user-location').innerText = area;
                } else {
    ...
    function error() {
        document.getElementById('user-location').innerText = "Permission denied";
    }
});