...
class UsersController extends Controller
{
    ...
    public function generateUniqueReferralCode(int $length = 10): string
    {
        do {            
    ...
    public function editStore(int $id = null): Renderable
    {
        $this->checkAuthorization(Auth::user(), ['user.edit']);
        ...
        return view('backend.pages.users.createOrUpdateStore', [            
            'user' => $user,
            'breadcrumbs' => [
                'title' => $title,
                'items' => [
    ...
    public function saveStore(Request $request): RedirectResponse
    {
        ...
        $this->checkAuthorization(Auth::user(), ['user.create', 'user.edit']);
        ...
        $validated = $request->validate([
            ...
            'store_logo'    => 'nullable|file|mimes:jpg,jpeg,png,gif|max:2048',
        ]);

        $user = User::findOrFail($validated['user_id']);

        if ($request->hasFile('store_logo')) {
            $validated['store_logo'] = $request->file('store_logo')->store('stores/logos', 'public');
        }else{
            $validated['store_logo'] = $user->storeDetail->store_logo;
        }

        $storeDetail = $this->userService->createOrUpdateStoreDetails($validated);

        $this->storeActionLog(ActionType::CREATED, ['store' => $storeDetail]);

        return redirect()->route('admin.users.index')->with('success', __('Store details have been saved.'));
    }
}
